...
import (
	...
	merchantRepo "github.com/faruqfadhil/hybrida/internal/repository/inmem/merchant"
...
func main() {
	...
	initMerchants := []*merchant.Merchant{
		{
			ID:          "MERCH-1",
			Name:        "Matahari",
	...
	merchantRepository := merchantRepo.NewMerchantRepository(initMerchants)
	merchantSvc := merchant.NewMerchantService(merchantRepository)
	productRepository := productRepo.NewProductRepository(initProducts)
	productSvc := product.NewProductService(productRepository, merchantSvc)
